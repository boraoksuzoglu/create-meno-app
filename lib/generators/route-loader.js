  ...
  for (const layer of router.stack) {
    ...
    if (layer.route) {
      for (const routeLayer of layer.route.stack) {
        if (typeof routeLayer.handle === 'function' && routeLayer.handle.constructor.name === 'AsyncFunction') {
          routeLayer.handle = asyncWrap(routeLayer.handle);
        }
      }
    }
    // A middleware layer (e.g. router.use(subRouter))
    if (typeof layer.handle === 'function' && layer.handle.constructor.name === 'AsyncFunction') {
