...
const run = async () => {
  ...
  const password = process.env.ADMIN_PASSWORD || await ask('Admin password (min 8 chars, upper+lower+digit): ');

  if (!email || !password || password.length < 8 || !/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/\\d/.test(password)) {
    console.error('Invalid email or password (min 8 chars, must include upper, lower, digit).');
    process.exit(1);
  }
