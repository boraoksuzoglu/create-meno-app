...
export function generatePackageJson(config) {
  ...
  if (includeAuth && socialProviders.includes('google')) {
    deps['google-auth-library'] = v('google-auth-library', '^9.15.1');
  }
  if (includeAuth && socialProviders.includes('apple')) {
    deps['apple-signin-auth'] = v('apple-signin-auth', '^2.0.0');
  }
  if (includeRateLimit) {
  ...
  if (isTs) {
    ...
    if (includeEmail) devDeps['@types/html-to-text'] = v('@types/html-to-text', '^9.0.4');
  }
  ...
  if (includeJest) {
    ...
    if (isTs) {
      devDeps['@types/supertest'] = v('@types/supertest', '^6.0.2');
      // Transpile TypeScript test sources for Jest (ESM-compatible, fast).
      devDeps['@swc/jest'] = v('@swc/jest', '^0.2.39');
      devDeps['@swc/core'] = v('@swc/core', '^1.13.5');
    }
  }
  ...
  if (includeAuth) {
    scripts['create:admin'] = isTs
      ? 'tsx --tsconfig tsconfig.json src/scripts/create-admin.ts'
      : 'node --import ./src/utils/path-loader.js src/scripts/create-admin.js';
  }

  if (includeMdDocs) {
    scripts['docs'] = isTs
      ? 'tsx --tsconfig tsconfig.json src/scripts/generate-docs.ts'
      : 'node --import ./src/utils/path-loader.js src/scripts/generate-docs.js';
  }
