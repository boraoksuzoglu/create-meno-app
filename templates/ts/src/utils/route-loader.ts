 ...
 */
const asyncWrap = (fn: (...args: any[]) => any) => (req: any, res: any, next: any) => {
 ...
 */
const wrapRouterHandlers = (router: Router) => {
 ...
 * Auto Route Loader
 * ─────────────────
 * Scans src/modules for files matching *.routes.ts and mounts them
 * automatically on Express based on their directory name.
 *
 * Convention:
 *   src/modules/<name>/<name>.routes.ts  →  app.use('/<name>', router)
 *
 ...
 *   // @no-auto-load
 */
export const loadRoutes = async (app: Express): Promise<string[]> => {
