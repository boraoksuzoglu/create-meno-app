 ...
 *
 * All shared parsing/inference lives in src/utils/doc-introspect.ts (also used by
...
interface ModuleSummary extends ModuleInfo {
  count: number;
}
type ModelInfo = ModelDoc & { slug: string };
...
const toTitleCase = (s: string): string =>
  s.split(/[-_]/).map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
const tableSafe = (s: string): string => String(s).replace(/\|/g, '\\|');
const json = (obj: unknown): string => '```json\n' + JSON.stringify(obj, null, 2) + '\n```\n\n';
...
const readModuleDesc = (src: string): string => {
...
// ── Discovery ───────────────────────────────────────────────────────────────
const discoverModules = (): ModuleInfo[] => {
...
const discoverModels = (): ModelInfo[] => {
  if (!fs.existsSync(SRC_MODELS)) return [];
  const out: ModelInfo[] = [];
  for (const fileName of fs.readdirSync(SRC_MODELS).filter((f) => f.endsWith(`.model.${EXT}`)).sort()) {
    const slug = fileName.replace(`.model.${EXT}`, '');
    const parsed = parseModel(path.join(SRC_MODELS, fileName));
...
// ── Markdown builders ─────────────────────────────────────────────────────────
const buildModuleDoc = async (mod: ModuleInfo, endpoints: Endpoint[]): Promise<string> => {
...
const buildModelDoc = (model: ModelInfo): string => {
  ...
  const refs = model.fields.filter((f: Field) => f.ref);
  if (refs.length) {
    md += '## Relationships\n\n';
    for (const f of refs) md += `- \`${f.name}\` → [${f.ref}](./${toPascalCase(f.ref as string)}.md)\n`;
    md += '\n';
  }
...
const buildModulesIndex = (summary: ModuleSummary[]): string => {
...
const buildModelsIndex = (models: ModelInfo[]): string => {
...
const buildDocsIndex = (moduleCount: number, modelCount: number): string => `# Documentation
...
// ── Collect → write / check ────────────────────────────────────────────────────
const collectFiles = async (): Promise<{
  ...
  for (const mod of modules) {
    ...
    files.set(
      path.join(OUT_DIR, 'modules', `${toPascalCase(mod.dirName)}.md`),
      await buildModuleDoc(mod, endpoints)
    );
